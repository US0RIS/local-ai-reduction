...
class QuantizedHeadBasisQ4:
    ...
    @property
    def gram_inv(self):
        return None if self.gram_inv_fp16 is None else self.gram_inv_fp16.float()

def quantize_head_basis_q4(basis:torch.Tensor,*,store_gram_inverse:bool=True)->QuantizedHeadBasisQ4:
    """Quantize [heads,rank,head_dim] basis and optionally store FP16 inverse Gram.

    If B_hat is the dequantized stored basis, G^-1=(B_hat B_hat^T)^-1 is
    required for exact orthogonal projection in latent coordinates. For keys it
    corrects q_lat^T k_lat. For values the reconstruction operator is
    B_hat^+ = B_hat^T G^-1, implemented as v_lat @ G^-1 @ B_hat.
    """
    ...
    if store_gram_inverse:
        grams=bh@bh.transpose(-1,-2);eye=torch.eye(r,dtype=torch.float32,device=bh.device).expand(h,r,r);diag=torch.diagonal(grams,dim1=-2,dim2=-1).mean(-1).clamp_min(1e-8)
        metric=torch.linalg.inv(grams+eye*(diag[:,None,None]*1e-5)).to(torch.float16).contiguous()
...
class LatentQ2KV:
    def __init__(self,k_basis:torch.Tensor,v_basis:torch.Tensor):
        self.kq=quantize_head_basis_q4(k_basis) if k_basis.ndim==3 else None
        self.vq=quantize_head_basis_q4(v_basis) if v_basis.ndim==3 else None
...
def cache_bytes(*,layers:int,seq:int,kv_heads:int,head_dim:int,rank:int,bits:int=2,scale_bytes_per_vector:int=4,basis_bits:int=4,basis_scale_bytes:int=2):
    vectors=layers*seq*kv_heads*2;latent_payload=math.ceil(vectors*rank*bits/8);quant_meta=vectors*scale_bytes_per_vector;basis_values=layers*kv_heads*2*rank*head_dim;basis_payload=math.ceil(basis_values*basis_bits/8);basis_scales=layers*kv_heads*2*rank*basis_scale_bytes;return latent_payload+quant_meta+basis_payload+basis_scales
...
def kivi_latent_cache_bytes(*,layers:int,seq:int,kv_heads:int,head_dim:int,rank:int,bits:int=2,group_tokens:int=64,basis_bits:int=4,basis_scale_bytes:int=2,tail_fp16:bool=False,key_metric_fp16:bool=True,value_metric_fp16:bool=True):
    vectors=layers*seq*kv_heads;payload=2*math.ceil(vectors*rank*bits/8);key_groups=layers*kv_heads*math.ceil(seq/group_tokens);key_meta=key_groups*rank*4;value_meta=vectors*4;basis_values=layers*kv_heads*2*rank*head_dim;basis_payload=math.ceil(basis_values*basis_bits/8);basis_scales=layers*kv_heads*2*rank*basis_scale_bytes;key_metric=layers*kv_heads*rank*rank*2 if key_metric_fp16 else 0;value_metric=layers*kv_heads*rank*rank*2 if value_metric_fp16 else 0;tail=layers*kv_heads*2*group_tokens*rank if tail_fp16 else 0;return payload+key_meta+value_meta+basis_payload+basis_scales+key_metric+value_metric+tail
