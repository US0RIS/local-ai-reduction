...
def pack_q2_rows_fp8meta(x:torch.Tensor):
    """Q2 coefficients with one E4M3-FN min and scale byte per vector."""
...
class QuantizedHeadBasisQ4:
    ...
    metric_inv_fp16:torch.Tensor|None=None
    @property
    def storage_bytes(self):
        n=self.packed.numel()*self.packed.element_size()+self.scale.numel()*self.scale.element_size()
        if self.metric_inv_fp16 is not None:n+=self.metric_inv_fp16.numel()*self.metric_inv_fp16.element_size()
        return n
    @property
    def metric_inv(self):return None if self.metric_inv_fp16 is None else self.metric_inv_fp16.float()
...
def quantize_head_basis_q4(basis:torch.Tensor,*,store_metric:bool=True)->QuantizedHeadBasisQ4:
    ...
    if store_metric:
        grams=bh@bh.transpose(-1,-2);eye=torch.eye(r,dtype=torch.float32,device=bh.device).expand(h,r,r);scale=torch.diagonal(grams,dim1=-2,dim2=-1).mean(-1).clamp_min(1e-8);grams=grams+eye*(scale[:,None,None]*1e-5);metric=torch.linalg.inv(grams).to(torch.float16).contiguous()
...
class LatentQ2KV:
    def __init__(self,k_basis:torch.Tensor,v_basis:torch.Tensor,*,fp8_metadata:bool=False):
        self.kq=quantize_head_basis_q4(k_basis,store_metric=True) if k_basis.ndim==3 else None;self.vq=quantize_head_basis_q4(v_basis,store_metric=True) if v_basis.ndim==3 else None;self.k_basis=(self.kq.dequantized if self.kq else k_basis.float().contiguous());self.v_basis=(self.vq.dequantized if self.vq else v_basis.float().contiguous());self.fp8_metadata=fp8_metadata
...
class KIVILatentQ2KV:
    ...
    def attention_head(self,q:torch.Tensor,ek:EncodedKeyChannels,ev:EncodedLatent,head:int):
        kl=unpack_q2_key_channels(ek.p,ek.mn,ek.scale,ek.rank,ek.tokens,ek.group_tokens);vl=unpack_q2_rows(ev.p,ev.mn,ev.scale,ev.cols);kb=self.k_basis[head];vb=self.v_basis[head];ql=(q.float()@kb.t())@self.kq.metric_inv[head];a=torch.softmax((kl@ql)/math.sqrt(q.shape[-1]),dim=-1);vlat=(a@vl)@self.vq.metric_inv[head];return vlat@vb
