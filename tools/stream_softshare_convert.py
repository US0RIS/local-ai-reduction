...
MATRIX_TYPES={
 'q_proj':'self_attn.q_proj.weight','k_proj':'self_attn.k_proj.weight','v_proj':'self_attn.v_proj.weight','o_proj':'self_attn.o_proj.weight',
 'gate_proj':'mlp.gate_proj.weight','up_proj':'mlp.up_proj.weight','down_proj':'mlp.down_proj.weight',
}
LAYER_RE=re.compile(r'^model\.layers\.(\d+)\.(.+)$')
Q4_HEAD=struct.Struct('<IIQ') # rows, cols, packed-byte-count
...
def make_page_plan(src:ShardedSafeTensorSource,L:int,ranks:dict[str,int]):
    pages=[];pid=1
    ...
    for typ,suf in MATRIX_TYPES.items():
        shape=src.info(layer_name(0,suf)).shape;r=ranks[typ]
        add('shared_base',f'@shared/{typ}',CODEC_Q4_ROW,FLAG_REQUIRED|FLAG_SHARED|FLAG_STREAMABLE,shape,matrix_type=typ)
        for l in range(L):
...
def fit_residual(residual:torch.Tensor,rank:int,oversample:int,niter:int,seed:int):
    ...
    root=S.clamp_min(0).sqrt();A=U*root;B=root[:,None]*V.t();return A.contiguous(),B.contiguous()

def parse_ranks(spec:str):
    if spec.isdigit():return {k:int(spec) for k in MATRIX_TYPES}
    out={}
    for item in spec.split(','):
        k,v=item.split('=',1);out[k.strip()]=int(v)
    missing=set(MATRIX_TYPES)-set(out)
    if missing:raise ValueError(f'missing ranks for {sorted(missing)}')
    return out

def convert(index:str|Path,out_path:str|Path,ranks:dict[str,int],oversample:int=8,niter:int=2,seed:int=1234):
    src=ShardedSafeTensorSource(index);L=discover(src);plan=make_page_plan(src,L,ranks);by_id={p['page_id']:p for p in plan}
    manifest={
      'architecture':'mistral','conversion':'SoftShare-10X two-pass initialization','source':{'safetensors_index':str(index),'full_source_file_required_locally':False},
      'softshare':{'layers':L,'ranks':ranks,'equation':'W_layer = shared_Q4 + A_Q4 @ B_Q4','residual_fit_reference':'dequantized stored shared_Q4','recovery_applied':False},
      'pages':plan,
    }
    peak_source=peak_shared=0;page_lookup={(p['role'],p.get('matrix_type'),p.get('layer')):p for p in plan}
    with LARCv2StreamWriter(out_path,len(plan),manifest) as wr:
        # Non-shared model state is copied/quantized one tensor at a time.
        ...
        # Per matrix family: pass 1 computes one shared base; pass 2 fits each
        # layer residual against the *stored/dequantized* shared Q4 base.
        for ti,(typ,suf) in enumerate(MATRIX_TYPES.items()):
            ...
            shared.div_(L);peak_shared=max(peak_shared,shared.numel()*shared.element_size());parts=q4_parts(shared);shared_hat=dequantize_q4_rows(*parts);blob=q4_blob_from_parts(*parts);p=page_lookup[('shared_base',typ,None)];wr.add_page(p['page_id'],p['codec_id'],p['flags'],blob,logical_length=shared.numel()*2);del shared,blob,parts
            for l in range(L):
                x=src.read_tensor(layer_name(l,suf));peak_source=max(peak_source,x.numel()*x.element_size());res=x.float().sub_(shared_hat);del x
                A,B=fit_residual(res,ranks[typ],oversample,niter,seed+ti*1000+l);del res
...
def main():
    ap=argparse.ArgumentParser(description='Two-pass tensor-range SoftShare converter; never requires a complete source checkpoint file locally.')
    ap.add_argument('--index',required=True,help='local path or HTTP(S) model.safetensors.index.json (remote shard endpoints must honor Range)')
    ap.add_argument('--output',required=True);ap.add_argument('--ranks',default='96',help='uniform integer or q_proj=...,k_proj=...,v_proj=...,o_proj=...,gate_proj=...,up_proj=...,down_proj=...')
    ap.add_argument('--oversample',type=int,default=8);ap.add_argument('--niter',type=int,default=2);ap.add_argument('--seed',type=int,default=1234);ap.add_argument('--report')
    a=ap.parse_args();report=convert(a.index,a.output,parse_ranks(a.ranks),a.oversample,a.niter,a.seed);text=json.dumps(report,indent=2)+'\n';print(text,end='')
