...
int main(){constexpr std::size_t M=1536,K=576,R=32;std::mt19937 g(9);std::normal_distribution<float>nd(0,1);std::vector<float>B(R*K),A(M*R),W(M*K),x(K),yd(M),yl(M),z(R),yfp(M);for(auto&a:B)a=nd(g)/std::sqrt(float(K));for(auto&a:A)a=nd(g)/std::sqrt(float(R));for(std::size_t i=0;i<M;i++)for(std::size_t j=0;j<K;j++){float s=0;for(std::size_t r=0;r<R;r++)s+=A[i*R+r]*B[r*K+j];W[i*K+j]=s+0.02f*nd(g);}for(auto&a:x)a=nd(g);auto qw=q4(W,M,K),qb=q4(B,R,K),qa=q4(A,M,R);double td=us([&]{larc::q4_gemv(qw.v(),x.data(),yd.data());});double tl=us([&]{larc::q4_projected_gemv(qb.v(),qa.v(),x.data(),z.data(),yl.data());});double se=0,sp=0,sefp=0,spfp=0;for(std::size_t i=0;i<M;i++){double exact=0;for(std::size_t j=0;j<K;j++)exact+=double(W[i*K+j])*x[j];yfp[i]=float(exact);double e=double(yd[i])-yl[i];se+=e*e;sp+=double(yd[i])*yd[i];double ef=double(yfp[i])-yl[i];sefp+=ef*ef;spfp+=double(yfp[i])*yfp[i];}auto bytes=[](const OQ&o){return o.p.size()+o.s.size()*sizeof(std::uint16_t);};auto db=bytes(qw),lb=bytes(qb)+bytes(qa);std::printf("dense_q4_bytes=%zu larc_factor_bytes=%zu resident_reduction=%.6f dense_us=%.6f larc_us=%.6f speedup=%.6f output_nmse_vs_dense_q4=%.9f output_nmse_vs_fp32=%.9f scratch_bytes=%zu\n",db,lb,double(db)/lb,td,tl,td/tl,se/sp,sefp/spfp,larc::projected_scratch_bytes(qb.v()));}
